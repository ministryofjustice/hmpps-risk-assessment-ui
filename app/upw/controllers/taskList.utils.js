...
const getDeclarationStatus = (answers, otherSections, declarationField) => {
  ...
  return checkDeclarationIsSigned(answers, declarationField, 'COMPLETE')
    ? 'COMPLETE'
    : 'INCOMPLETE'
}
...
const getTaskList = (baseUrl = '', steps = {}, answers = {}) => {
  ...
  const tasks = [
    {
      ...
      items: [
        getTask(answers, baseUrl, steps, 'individuals-details'),
      ]
    },
    {
      heading: {
  ...
  const declaration = {
    ...
    items: [
      getDeclarationTask(answers, baseUrl, steps, 'pdf-preview-and-declaration', tasks, 'declaration')
    ],
  }
